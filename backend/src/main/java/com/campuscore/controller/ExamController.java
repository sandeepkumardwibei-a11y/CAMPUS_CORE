...
public class ExamController {
    ...
    // 🔐 SECURED: Only administrators or the exam controller may cancel a scheduled exam.
    @PutMapping("/{examId}/cancel")
    @PreAuthorize("hasAnyRole('ADMIN', 'EXAM_CONTROLLER')")
    public ResponseEntity<ApiResponse<ExamDto.Response>> cancelExam(@PathVariable Long examId) {
        log.info("Processing cancelExam endpoint request for examId: {}", examId);

        ExamDto.Response response = examService.cancelExam(examId);

        log.info("Successfully processed cancelExam endpoint request for examId: {}", examId);
        return ResponseEntity.ok(ApiResponse.success(response, "Exam cancelled successfully."));
    }
