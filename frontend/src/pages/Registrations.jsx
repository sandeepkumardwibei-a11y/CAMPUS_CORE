...
function StaffView() {
  ...
  const runScope = async () => {
    ...
    try {
      ...
      // No match (or lookup failure): clear stale results so the table shows

      // a "No results found" empty state instead of the previous data + a popup.
...
<Card className="p-4 mb-5 flex flex-wrap items-end gap-3">
          ...
          {tab === 'student' && <div className="w-40"><span className="label">Student ID</span><Input type="number" min={1} max={999999} value={q.studentId} onChange={(e) => setQ({ ...q, studentId: e.target.value })} placeholder="5" /></div>}

          {tab === 'id' && <div className="w-40"><span className="label">Registration ID</span><Input type="number" min={1} max={999999} value={q.regId} onChange={(e) => setQ({ ...q, regId: e.target.value })} placeholder="1" /></div>}

          {tab === 'course' && <div className="w-40"><span className="label">Course ID</span><Input type="number" min={1} max={999999} value={q.courseId} onChange={(e) => setQ({ ...q, courseId: e.target.value })} placeholder="1" /></div>}
